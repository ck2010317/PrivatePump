...
pub fn handler(ctx: Context<ErBuy>, sol_amount: u64, min_tokens_out: u64) -> Result<()> {
    ...
    // Transfer SOL to bonding curve (minus fee)
    ...
    // On ER, fees accumulate in the bonding curve account and are
    // transferred to fee_recipient on commit/undelegate.
    // We still collect the fee from the buyer into the curve so it's not lost.
    if fee > 0 {
        anchor_lang::system_program::transfer(
            CpiContext::new(
                ctx.accounts.system_program.to_account_info(),
    ...
    // Update reserves (real_sol tracks actual SOL minus fee, virtual tracks full amount for pricing)
